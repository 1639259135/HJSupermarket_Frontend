    ...
    <el-card v-loading="loading">
      <template #header>
        <div style="display:flex;align-items:center;gap:12px">
          <el-button @click="$router.push('/sales')" text><el-icon><ArrowLeft /></el-icon></el-button>
          <span>销售单详情 — {{ detail.saleNo }}</span>
        </div>
      </template>
      ...
      <el-table :data="detail.items || []" border stripe>
        ...
        <el-table-column label="小计" width="100"><template #default="{row}">¥{{ (row.quantity*row.price).toFixed(2) }}</template></el-table-column>
      </el-table>
    </el-card>
  </div>
</template>
...
onMounted(async () => {
  try { detail.value = await getSaleDetail(Number(route.params.id)) } catch {} finally { loading.value = false }
})
</script>
